...
pub struct TimestampStrftimeRange(String);

impl TryFrom<&str> for TimestampStrftimeRange {
    type Error = chrono::format::ParseError;

    fn try_from(value: &str) -> Result<Self, Self::Error> {
        NaiveDateTime::parse_from_str(value, "%Y%m%d%H%M%S")?;
        Ok(TimestampStrftimeRange(value.to_owned()))
    }
}

impl fmt::Display for TimestampStrftimeRange {
...
impl TimestampGenerator {
    ...
    fn adjust_start_date(&mut self) {
        ...
        let start_adjustment =
            (self.start_date.minute() as i64 * 60 + self.start_date.second() as i64) % interval_seconds;
    ...
    fn adjust_end_date(&mut self) {
        ...
        let end_adjustment = interval_seconds
            - ((self.end_date.minute() as i64 * 60 + self.end_date.second() as i64) % interval_seconds);
        if end_adjustment != interval_seconds {
            self.end_date += Duration::try_seconds(end_adjustment).expect("Couldn't add seconds to end date");
        }
    }

    pub fn generate(&self) -> Vec<TimestampStrftimeRange> {
        ...
        while date <= self.end_date {
            if let Ok(ts) =
                TimestampStrftimeRange::try_from(date.format("%Y%m%d%H%M%S").to_string().as_str())
            {
                timestamps.push(ts);
            }
...
mod tests {
    ...
    fn test_timestamp_strftime_range_try_from_valid() {
        ...
        let result = TimestampStrftimeRange::try_from(valid_str);
        assert!(result.is_ok());
        let timestamp = result.unwrap();
        assert_eq!(timestamp.to_string(), "TimestampStrftimeRange (20230101123045)");
    }

    #[test]
    fn test_timestamp_strftime_range_try_from_invalid() {
        let invalid_str = "not a timestamp";
        let result = TimestampStrftimeRange::try_from(invalid_str);
    ...
    fn test_interval_to_duration_five_minutes() {
        ...
        assert_eq!(duration, Duration::try_seconds(5 * 60).expect("Failed to create duration for 5 minutes"));
    }
    ...
    fn test_interval_to_duration_thirty_minutes() {
        ...
        let duration = interval.to_duration();
        assert_eq!(duration, Duration::try_seconds(30 * 60).expect("Failed to create duration for 30 minutes"));
    }
    ...
    fn test_adjust_start_date() {
        let start_date_opt = NaiveDate::from_ymd_opt(2023, 4, 1)
            .and_then(|date| date.and_hms_opt(0, 3, 0)); // 3 minutes past the hour

        // Ensure start_date is valid before proceeding
        let start_date = start_date_opt.expect("Invalid start date");
        let end_date = start_date + Duration::try_hours(1).expect("Failed to create duration of 1 hour");
    ...
    fn test_adjust_end_date() {
        ...
        // Ensure start_date is valid before proceeding
        let start_date = start_date_opt.expect("Invalid start date");
        let end_date = start_date + Duration::try_minutes(58).expect("Failed to create duration of 58 minutes");
        let mut generator = TimestampGenerator::new(start_date, end_date, Interval::FiveMinutes);
